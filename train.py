...
def parse_arguments():
    ...
    p.add_argument('-hidden_size', type=int, default=512,
                   help='the number of RNN hidden node.')
    ...
    p.add_argument('-ctc_layers', type=int, default=2,
                   help='the layer number of ctc segmentation network.')
    p.add_argument('-encoder_layers', type=int, default=2,
                   help='the layer number of encoder.')
    p.add_argument('-decoder_layers', type=int, default=1,
                   help='the layer number of decoder.')
    return p.parse_args()

def evaluate_ctc(model, e, val_iter, vocab_size, ZH_WORD, seg_result_filename = None):
    if seg_result_filename == None:
        f_out = open('val_result' + str(e) + '.txt', 'w', encoding='utf-8')
    else:
...
def evaluate_nmt(model, e, val_iter, vocab_size, ZH_WORD, EN_WORD, nmt_result_filename = None):
    if nmt_result_filename == None:
        f_out = open('val_nmt_result' + str(e) + '.txt', 'w', encoding='utf-8')
    else:
...
def evaluate_combine(model, e, val_iter, vocab_size, ZH_CHA, EN_WORD, nmt_result_filename = None):
    if nmt_result_filename == None:
        f_out = open('val_nmt_result'+ str(e) + '.txt', 'w', encoding='utf-8')
    else:
        f_out = open(nmt_result_filename, 'w', encoding = 'utf-8')
...
def evaluate_update_twoLoss(model, e, val_iter, zh_word_size, en_word_size, ZH_CHA, ZH_WORD, EN_WORD, seg_result_file = None, nmt_result_filename = None):
    if nmt_result_filename == None:
        f_out_nmt = open('val_nmt_result' + str(e) + '.txt', 'w', encoding='utf-8')
    else:
        f_out_nmt = open(nmt_result_filename, 'w', encoding = 'utf-8')
    if seg_result_file == None:
        f_out_ctc = open('val_ctc_result' + str(e) + '.txt', 'w', encoding='utf-8')
    else:
...
def main_ctc(args, f_out, f_inf, hidden_size, embed_size):
    #准备数据
    train_iter, val_iter, test_iter, Ti_CHA, Ti_WORD, ti_cha_size, ti_word_size = prepareDate(args, f_out)
    ...
    encoder = Encoder_Combine(hidden_size, hidden_size,
        n_layers=args.encoder_layers, dropout=0.5)
    zh_word_size = ti_word_size
    decoder = Decoder(embed_size, hidden_size, zh_word_size,
        n_layers=args.decoder_layers, dropout=0.5)
    ctc_seg = CTC_Seg(ti_cha_size, embed_size, hidden_size, ti_word_size + 1, 
        n_layers=args.ctc_layers, dropout=0.5)
    seq2seq = Seq2Seq(args.mode, encoder, decoder, ctc_seg).cuda()
    optimizer = optim.Adam(seq2seq.parameters(), lr=args.lr)
    print(seq2seq)
    f_out.write(str(seq2seq))
    f_out.flush()
    ...
    for e in range(1, args.epochs+1):
        train_ctc(e, f_out, f_inf, seq2seq, optimizer, train_iter, args.grad_clip, ti_word_size, Ti_CHA, Ti_WORD)
        val_loss, val_precision, val_recall, val_f_measure = evaluate_ctc(seq2seq, e, val_iter, ti_word_size, Ti_WORD)
    ...
    test_loss, test_precision, test_recall, test_f_measure = evaluate_ctc(seq2seq, e, test_iter, ti_word_size, Ti_WORD, args.seg_result_file)
...
def main_nmt(args, f_out, hidden_size, embed_size):
    #准备数据
    train_iter, val_iter, test_iter, Ti, ZH, ti_size, zh_size = prepareDate(args, f_out)

    print("Instantiating models...")
    f_out.write("Instantiating models...\n")
    f_out.flush()
    encoder = Encoder(ti_size, embed_size, hidden_size,
                      n_layers=args.encoder_layers, dropout=0.5)
    ...
    for e in range(1, args.epochs+1):
        train_nmt(e, f_out, seq2seq, optimizer, train_iter,
            zh_size, args.grad_clip, Ti, ZH)
        val_loss = evaluate_nmt(seq2seq, e, val_iter, zh_size, Ti, ZH)
    ...
    seq2seq.load_state_dict(torch.load(best_model_path))
    test_loss = evaluate_nmt(seq2seq, e, test_iter, zh_size, Ti, ZH, args.nmt_result_file)
...
def main_combine(args, f_out, hidden_size, embed_size):
    #准备数据
    train_iter, val_iter, test_iter, Ti_CHA, ZH_WORD, ti_cha_size, zh_word_size = prepareDate(args, f_out)
    ...
    for e in range(1, args.epochs+1):
        train_combine(e, f_out, seq2seq, optimizer, train_iter,
            zh_word_size, args.grad_clip, Ti_CHA, ZH_WORD)
        val_loss = evaluate_combine(seq2seq, e, val_iter, zh_word_size, Ti_CHA, ZH_WORD)
    ...
    seq2seq.load_state_dict(torch.load(best_model_path))
    test_loss = evaluate_combine(seq2seq, e, test_iter, zh_word_size, Ti_CHA, ZH_WORD, args.nmt_result_file)
    print("[TEST] loss:%5.2f" % test_loss)
    string = 'Test loss: ' + str(test_loss) + '\n'
    f_out.write(string)
    f_out.flush()
...
def main_update_twoLoss(args, f_out, f_inf, hidden_size, embed_size):
    #准备数据，加载训练ctc分词网络的数据（藏文音节-藏文词），和联合训练的数据（藏文音节-中文词）
    train_iter, val_iter, test_iter, Ti_CHA, ZH_WORD, Ti_WORD, ti_cha_size, zh_word_size, ti_word_size = prepareDate(args, f_out)
   ...
   #定义并初始化网络
    ...
    for e in range(1, args.epochs+1):
        train_update_twoLoss(e, f_out, f_inf, seq2seq, optimizer, train_iter, args.grad_clip, ti_word_size, zh_word_size, Ti_CHA, Ti_WORD, ZH_WORD)
        val_loss_ctc, val_loss_nmt, val_precision, val_recall, val_f_measure = evaluate_update_twoLoss(seq2seq, e, val_iter, ti_word_size, zh_word_size, Ti_CHA, Ti_WORD, ZH_WORD)
    ...
    #加载在开发及上性能最好的模型进行测试
    seq2seq.load_state_dict(torch.load(best_model_path))
    test_loss_ctc, test_loss_nmt, test_precision, test_recall, test_f_measure = evaluate_update_twoLoss(seq2seq, e, test_iter, ti_word_size, zh_word_size, Ti_CHA, Ti_WORD, ZH_WORD, args.seg_result_file, args.nmt_result_file)
...
if __name__ == "__main__":
    try:
        with open('log_hiddensize256', 'w', encoding='utf-8-sig') as f_out:
            main(f_out)
